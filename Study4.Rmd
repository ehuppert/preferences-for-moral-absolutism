...
#abs mean =   4.22, sd = 2.12, 201 
#flex mean =  3.59, sd = 1.63, 198
#pooled SD = sqrt(((2.12^2) + (1.63^2))/2)
sqrt((((2.12^2) + (1.63^2))/2)) #1.890939

#cohens = (mean - mean)/SD pooeld
(4.22-3.59)/1.890939 #0.33
...
d4 #Cohen's d abs. vs. flex. on morality (abs value)  = 0.34, enter this in power
...
```{r}
#Reported Model
rv_4.glm <- glm(rv ~ sometimes + rarely + behavlie 
...
table(s4$proclamation, s4$behavior, s4$rv)
table(s4$behavior, s4$rv) 
...
rv4.4.glm <- glm(rv ~ sometimes*pref.center,
              data=s4, family = binomial(link = "logit"))
summary(rv4.4.glm) 

#Frequencies
table(s4$rv, s4$pref, s4$sometimes)
#0 = verify, 1 = rely
table(s4$rv, s4$pref, s4$proclamation)
...
#Percentages that RELY on rarey
#always 1/4 25%
#often 1/9 11%
#sometimes 8/144 6%
#rarely 62/330 19%
