...
#Dummy to indicate if participants and targets shared honesty preferences
```{r}
#Subset of participants whose honesty preferences could match the target
s4match <- s4[s4$pref >= 3, ]

#Dummy indicator for matching preferences
s4match$match <- ifelse((s4match$never==1 & s4match$pref==5) | (s4match$rarely==1 & s4match$pref==4) | (s4match$sometimes==1 & s4match$pref==3), 1, 0)
...
#Preferences Regression - Hypocrisy Matching
#Model 1
hypocrisy_4.lm1 <- lm(Hypocrisy_composite4 ~ never + rarely + match, data= s4match)
summary(hypocrisy_4.lm1)

#Model 2
hypocrisy_4.lm2 <- lm(Hypocrisy_composite4 ~ never + rarely + match
                   + rarely*match + never*match, data= s4match)
...
#Preferences Regression - Morality Matching
#Model 1
moral_4.lm1 <- lm(Moral_composite4 ~ never + rarely + match, data= s4match)
summary(moral_4.lm1)

#Model 2
moral_4.lm2 <- lm(Moral_composite4 ~ never + rarely + match
                   + rarely*match + never*match, data= s4match)
summary(moral_4.lm2)
...
#Preferences Regression - Commitment to Honesty Matching
#Model 1
honest_4.lm1 <- lm(Honest_composite4 ~ never + rarely + match, data= s4match)
summary(moral_4.lm1)

#Model 2
honest_4.lm2 <- lm(Honest_composite4 ~ never + rarely + match
                   + rarely*match + never*match, data= s4match)
...
#Preferences Regression - Social Benefit (SI) Matching
#Model 1
SI.lm1 <- lm(SI_composite4 ~ never + rarely + match, data= s4match)
summary(SI.lm1)

#Model 2
SI.lm2 <- lm(SI_composite4 ~ never + rarely + match
                   + rarely*match + never*match, data= s4match)
summary(SI.lm2)
...
#Logistic Regression (trust) with own honesty preferences (matching)
#Model 1
rv4.4.glm.1 <- glm(rv_f ~ never + rarely + match,
              data=s4match, family = binomial(link = "logit"))
summary(rv4.4.glm.1)

#Model 2
rv4.4.glm.2 <- glm(rv_f ~ never + rarely + match
                   + never*match + rarely*match,
              data=s4match, family = binomial(link = "logit"))
summary(rv4.4.glm.2)
